...
def smallest_integer(numbers_list):
    odds = []
    for number in numbers_list:
        if number % 2 == 1:
            odds.append(number)
    # return min(odds)
    odds.sort() # sorts the list in an ascending order
    return odds[0] # selects the first element/integer

print(smallest_integer([12, 89, 45, 67, 2, 4, 6, 8, 0, -2, 99, 33, 13, 7]))