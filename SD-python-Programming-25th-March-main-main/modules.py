...
# Tecchnically speaking, a module is nothing more than just a Python file containing specific functions, variables or classes.Ofcourse these functions and other data types are not randomly grouped inside that file. They are grouped together to provide some functionality, which cn be used further inside other programs.
# There are alot of Python Modules-hundreds, maybe even thousands that can you can use into your programs. Some of them the default modules belonging to the python built-in namespace, and others can ve downloaded, unzipped and installed separately.
...
# Now, letrs consider the math module, which is a Python built-in module and see its contents, using the dir()function.Also don't forget to import its first using the import statement.
...
# The genenral rule for accessing module functions and variables when using 'import' statement is typing in the name of the module, a dot, and then the name of the function or the variable you need to access.
...
# On the other hand, if the file is not going to be executed as a stand-aloneapplication, instead it is going to be used as a module and imported somewhere else, then do not execute de code below the if statement.